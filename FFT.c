...
int main(int argc, char *argv[])   {
    ...
    if (argc < 3)   {
        ...
        printf("                 -t winType (0=rect. 1=Hanning (default))\n");
        printf("        flags (FFT->wav): -s samplingRate (original default)\n");
        printf("        Converts .wav(snd) to .fft, and .fft to .wav(snd)\n");
        exit(0);
    }
#ifdef LITTLENDIAN
    ...
    if (input && output)	{ 
		...
		if (argv[i][1] == 't')	{
		    winType = atoi(argv[i+1]);
		}
                i += 1;
            }
            if (wsize > fsize)      {
                printf("Setting Window Size to FFT Size\n");
                wsize = fsize;
            }
            if (hsize > wsize)      {
                printf("Setting Hop Size to Window Size\n");
                hsize = wsize;
            }
            fhdr.fftsize = fsize;
            fhdr.windowsize = wsize;
            fhdr.hopsize = hsize;
	    fhdr.winType = winType;
        }
        if (wavein = 1) {
            fwrite(&fhdr,4,7,output);
        }
        result = (float *) calloc(2,4 * fsize);
        data = (short *) malloc(4*length);
        window = (float *) malloc(4 * wsize);
        makewindow(winType,window, wsize);
...
//            fwrite(&hdr,44,1,output);
            length = fhdr.dlength;
            data = (short *) malloc(2);
            result = (float *) calloc(2,5 * (length+fsize));	// HACKOLA!!
